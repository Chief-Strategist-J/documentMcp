...
class DocumentController {
    constructor(databaseManager, schemaValidator) {
        ...
        this.cephStorage = null; // FIX: was private, now accessible from DocumentServer
        this.documentStorage = new DocumentStorage_1.DocumentStorage();
        this.documentStorage.initialize().catch(error => {
        ...
        if (process.env['USE_CEPH_STORAGE'] === 'true') {
            try {
                ...
                this.cephStorage.initialize().catch(error => {
                    console.error('Failed to initialize Ceph storage:', error);
                    this.cephStorage = null;
                });
            }
            catch (error) {
                console.error('Failed to create Ceph storage:', error);
                this.cephStorage = null;
            }
        }
    }
    ...
    async addSectionData(req, res) {
        try {
            ...
            // FIX: use updateSectionData (dedicated method) instead of clobbering
            // the whole section object with a spread that drops the data field
    ...
    async getSectionData(req, res) {
        try {
            ...
            // FIX: read from section.data (populated by updateSectionData)
