...
interface ICephStorage {
  ...
  listDocuments(): Promise<Array<{documentId: string, format: string, key: string, createdAt: string}>>;
}
 ...
 */
export class CephStorage implements ICephStorage {
  ...
  async saveDocument(documentId: string, format: string, buffer: Buffer): Promise<string> {
    ...
    try {
      ...
      const key = `documents/${documentId}/${uuidv4()}.${format === 'word' ? 'docx' : 'pdf'}`;
   ...
   */
  async getDocument(documentId: string, format: string): Promise<Buffer | null> {
    // Validate inputs
    if (!documentId || typeof documentId !== 'string') {
      throw new Error('Document ID must be a non-empty string');
    }
   ...
   */
  async listDocuments(): Promise<Array<{documentId: string, format: string, key: string, createdAt: string}>> {
    try {
      const listCommand = new ListObjectsCommand({
        Bucket: this.bucketName,
        Prefix: 'documents/',
